...
def generate_data(params:dict):
    ...
    file_target_name, file_target_name_assort, shops_count, items_count, date_start, date_end, random_seed = params.values()
    ...
    """
    change_days, shuffle  = [0], random.randint(0, days//30)
    for _ in range(shuffle): change_days.append(int(random.random()*days))
    change_days.sort()
    ...
    item_counts = {item: generate_counts_for_item(dt_list=dt_list,
        start_count=start_counts[i], days=days, change_days=change_days,
        total_trend=1) for i, item in enumerate(items)}
        # total_trend=1+(2*random.random()-1)/100000) for i, item in enumerate(items)}
    ...
    pd.DataFrame(assort_future_list).rename(columns={0:'Shop', 1:'Item', 2:'Date_1', 3:'Date_2'}).to_csv(file_target_name_assort, index=False)
